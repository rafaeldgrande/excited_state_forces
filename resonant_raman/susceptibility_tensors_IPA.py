...
parser.add_argument('--el_ph_file', default='elph_in_ph_basis.h5', help='elph coefficients in phonon basis. generated by ../post_processing/elph_in_ph_basis.py')
parser.add_argument('--eqp_file', default='eqp.dat', help='eqp.dat file in the fine grid (produced by absorption.flavor.x)')
parser.add_argument('--nval_in_eqp', type=int, default=-1, help='index of highest valence band in eqp.dat')
parser.add_argument('--dip_mom_noeh_file_b1', default='eigenvalues_b1_noeh.dat')
parser.add_argument('--dip_mom_noeh_file_b2', default='eigenvalues_b2_noeh.dat')
parser.add_argument('--dip_mom_noeh_file_b3', default='eigenvalues_b3_noeh.dat')
...
parser.add_argument('--vectorized_flavor_second_order', type=int, default=1, choices=[0, 1, 2], help='0: no vectorization, 1: vectorize over k,c,v (jmode loop kept), 2: vectorize over jmode+k,c,v (more memory)')
...
parser.add_argument('--limit_transitions', type=int, default=None,
                    help='Truncate to first N transitions (for quick tests)')
parser.add_argument('--flavor_energy_levels', type=int, default=1, choices=[1, 2], help='Flavor energy levels. 1 = GW, 2 = DFT')
args = parser.parse_args()

flavor_energy_levels = args.flavor_energy_levels
el_ph_file = args.el_ph_file
...
print('--- Options ---')
print(f'  el_ph_file.       : {el_ph_file}')
print(f'  eqp_file          : {eqp_file}')
print(f'  nval_in_eqp       : {nval_in_eqp}')
...
# ---------------------------------------------------------------------------
# 2. Load elph matrix elements in phonon basis
# ---------------------------------------------------------------------------
print(f'\nReading e-ph matrix elements from {el_ph_file}')
with h5py.File(el_ph_file, 'r') as hf:
    g_cond = hf['elph_cond'][:]           # (Nmodes, nk, nc, nc)
    g_val  = hf['elph_val'][:]            # (Nmodes, nk, nv, nv)
...
# ---------------------------------------------------------------------------
# 3. Parse eqp.dat → single-particle energies
# ---------------------------------------------------------------------------

print(f'\nReading energy levels from file {eqp_file}')
...
# slice bands
# print('bands_dft.shape =', bands_dft.shape)
# print('nval_index =', nval_index)

Edft_val  = bands_dft[:nval_index+1, :] # shape (nv, nk)
Eqp_val   = bands_qp[:nval_index+1, :] # (nv, nk)
Edft_cond  = bands_dft[nval_index+1:, :] # (nc, nk)
Eqp_cond   = bands_qp[nval_index+1:, :] # (nc, nk)

nc_eqp = Eqp_cond.shape[0]
nv_eqp = Eqp_val.shape[0]
print(f'  nk={nk_eqp}, nv={nv_eqp}, nc={nc_eqp}')

if flavor_energy_levels == 1: # gw
