...
independently: `--skip-interpolation` stops after writing `elph_coarse.h5`;
...
the coarse-grid `elph_coarse.h5`.
...
- Cell, k-points, `nbnd`, and Nval (highest occupied band index): from
  `WFN_co.h5` (`--wfn_co`, preferred — authoritative, no guessing) or from
...
- `WFN_fi.h5` (`--wfn_fi`) for finite-q interpolation (auto-discovered next to `--dtmat` if not given)
- `eqp.dat` (`--eqp`, optional) — fine-grid QP energies from `inteqp.x`, for QP rescaling

**Usage:**

```bash
# Full pipeline: assemble XML -> elph_coarse.h5, interpolate -> elph_fine.h5
python elph_xml_to_h5.py --elph_dir _ph0/mos2.phsave --wfn_co WFN_co.h5 \
    --dtmat dtmat --wfn_fi WFN_fi.h5 --eqp eqp.dat
...
python elph_xml_to_h5.py --elph_coarse elph_coarse.h5 --wfn_co WFN_co.h5 \
    --dtmat dtmat --wfn_fi WFN_fi.h5
...
python elph_xml_to_h5.py --elph_dir _ph0/mos2.phsave --wfn_co WFN_co.h5 --Nval 13

# Disable acoustic sum rule (default: ASR applied, assembly stage only)
python elph_xml_to_h5.py --elph_dir _ph0/mos2.phsave --wfn_co WFN_co.h5 --no-ASR
```
...
| `--wfn_co` | `None` | Path to `WFN_co.h5`. If given and found, cell, k-points, `nbnd`, and Nval are read directly from its `mf_header` |
| `--qe_input` | `scf.in` | QE pw.x input file used for cell/k-points/`nbnd`/Nval when `--wfn_co` is not given or not found |
| `--Nval` | `None` | Manual override for Nval (highest occupied band index, QE `nbnd` convention). Takes precedence over `--wfn_co`/`--qe_input` either way |
| `--elph_coarse` | `elph_coarse.h5` | Output path when `--elph_dir` is given; required input path (must exist) when `--elph_dir` is omitted |
| `--elph_fine` | `elph_fine.h5` | Output HDF5 filename for the fine-grid (interpolated) el-ph file |
| `--skip-interpolation` | off | Stop after writing `--elph_coarse`. `--dtmat`/`--wfn_fi`/`--eqp` are ignored and `--elph_fine` is NOT written |
| `--dtmat` | `dtmat` | Path to BerkeleyGW `dtmat` binary |
| `--wfn_fi` | `WFN_fi.h5` | Path to `WFN_fi.h5` (required for finite-q interpolation; auto-discovered next to `--dtmat` if not found) |
| `--real` | off | Use real-flavor `dtmat` (default: complex) |
| `--wfn_fi_same_wfn_co` | off | Set when `WFN_fi == WFN_co` (el-ph coefficients computed directly on the fine grid). Skips reading `dtmat` and uses the identity matrix for the coarse-to-fine wavefunction overlaps; fine k-points default to the coarse grid if `--wfn_fi` is not found |
...
**Output HDF5 layout** (`elph_coarse.h5` and `elph_fine.h5` share the exact
...
| Argument | Default | Description |
|----------|---------|-------------|
| `--elph_fine` | `elph_fine.h5` | Input from `elph_xml_to_h5.py` |
...
- **Cartesian vs. mode basis**: both `elph_coarse.h5` and `elph_fine.h5` contain both. The Cartesian basis (`_cart` datasets) is needed for forces in the atomic basis; the mode basis (`_mode` datasets) is needed for forces resolved by phonon mode and frequency. Both are used by `excited_forces.py`.
